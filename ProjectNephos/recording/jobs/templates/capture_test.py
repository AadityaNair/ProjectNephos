...
class TestCaptureStreamJob:
    ...
    def test_create_command_from_config(self):
        ...
        with mock.patch(
            "unav.recordingmonitor.jobs.templates._common.get_session"
        ) as patch_get_session:
        ...
        assert (
            cmd_cap.command
            == "multicat -d 1215000000 -u @127.0.0.1:54321 name-capturing-program.mpg"
        )
        ...
        assert (
            cmd_aft.command
            == "proga --sl1=name-capturing-program.mpg --plain=123 --id=123 --slug=name-capturing-program"
        )

        cmd_aft_str = str(cmd_aft)
        assert (
            cmd_aft_str
            == "proga --sl1=name-capturing-program.mpg --plain=123 --id=123 --slug=name-capturing-program > /dev/null"
        )
