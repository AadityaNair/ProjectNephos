...
class CommandError(BaseError):
    ...
    def __init__(
        ...
        msg = (
            ...
            "stderr: [{stderr}]"
        ).format(command=str(command), rc=rc, stdout_way=stdout_way, stderr=stderr)
...
class Command:
    ...
    def __str__(self):
        ...
        res = "{tm}{cmd}{out}".format(tm=_tm, cmd=self.command, out=str(self.out))
        ...
        with self.out as outfd, self.inp as inpfd:
            ...
            with Popen(
                ...
                try:
                    ...
                    os.killpg(
                        pid, signal.SIGTERM
                    )  # SIGINT)  # send signal to the process group
...
class CaptureCommand(Command):
    ...
    def __init__(self, channel_ip, ifaddr, cwd="", out=None, timeout_sec=None):
        ...
        super().__init__(cmd=cmd, cwd=cwd, out=None, timeout_sec=None)
...
class VideoInfoCommand(Command):
    ...
    def __init__(self, inp, cwd="", timeout_sec=10):
        ...
        super().__init__(cmd=cmd, cwd=cwd, out="PIPE", timeout_sec=timeout_sec)
...
class ExtractCaptionsCommand(Command):
    ...
    def __init__(
        ...
        if not ifst.path:
            raise ValueError(
                "parameter `inp` of ExtractCaptionsCommand MUST be a file-path"
            )
        ...
        cargs = (
            "-ttxt -autoprogram -UCLA -noru -utf8 --nofontcolor -parsepat -parsepmt -stdout"
        )
