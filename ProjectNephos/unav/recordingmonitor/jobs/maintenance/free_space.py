...
def start(app_config):
    config = {"path": app_config.get("capture.paths.jobsRoot")}

    total, used, free = shutil.disk_usage(config["path"])

    return {
        "kind": FreeSpaceJobResultProcessor.KIND,
        "data": {"path": config["path"], "used": used, "free": free, "total": total},
    }
...
class FreeSpaceJobResultProcessor(BaseJobResultProcessor):
    ...
    def handle_data(self, data):
        ...
        if free < self._min_bytes or free_percent < self._min_percent:
