...
class ChannelChecker:
    ...
    def __enter__(self):
        ...
        self.cwd = os.path.join(
            self.jobs_root, "maintenance", "channel_on_air", str(self.ID)
        )
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        log.info("status of [%s] checked: [%s]", self.channel.name, cs.status)
        ...
        self.res = {
            ...
            "ts": cs.ts,
            "old": {"status": old_status, "error": old_error},
            "new": {"status": cs.status, "error": cs.error},
        }
    ...
    def check(self):
        ...
        if ts_file_size < 10:
            raise ChannelStreamIsEmptyError(
                (
                    "Channel's stream is empty:"
                    " captured {:d} bytes during {:d} seconds"
                ).format(ts_file_size, self.DEMO_CAPTURING_TIMEOUT_SEC)
            )
        ...
        if vformat_name != "mpegts":
            raise ChannelCorruptedError(
                "Unexpected stream format: {}".format(vformat_name)
            )

            # TEST 3: ccexctractor
        ccexctractor_cmd = Command(
            "ccextractor -xmltv -out=null {}".format(ts_file), cwd=self.cwd
        )
        ccexctractor_cmd.run()

        # output-file created by ccextractor:
        # TO-DO - UPLOAD EPG XML FILE SOMEWHERE, IF NEEDED
...
def start(app_config):
    ...
    for channel_id in ids:
        ...
        #   }
        # }
    return {"kind": ChannelOnAirJobResultProcessor.KIND, "data": _channels_result}
...
class ChannelOnAirJobResultProcessor(BaseJobResultProcessor):
    ...
    def handle_data(self, data):
        ...
        if something_changed > 0:
            ...
            # )
            log.info(
                "ChannelOnAirJobResultProcessor: there still are channels in down state"
            )
        else:
