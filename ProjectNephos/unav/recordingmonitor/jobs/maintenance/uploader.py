...
class UploaderTask:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type is None:
            ...
            self.log.error(
                "Uploader: error occured", exc_info=(exc_type, exc_val, exc_tb)
            )
    ...
    def process_one_file(self, meta):
        ...
        # STEP: get video info
        # ======================================================================
        res_capinfo = self._run_cmd(VideoInfoCommand(inp=filename_out, cwd=self.cwd))

        # 1a Duration
        _video_duration_seconds = float(
            pydash.get(res_capinfo.stdout, "format.duration")
        )
        ...
        _scaled_size = self._get_scaled_picture_size(
            _picture_width, _picture_height, _picture_size
        )
        ...
        _meta_timezone = (
            job_params.get("timezone") or channel.meta_timezone
        )  # noqa: E272, E222
        _meta_teletext_page = (
            job_params.get("meta_teletext_page") or channel.meta_teletext_page
        )  # noqa: E272, E222
        # _meta_country_code =   job_params.get('meta_country_code')   or channel.meta_country_code      # noqa: E272, E222
        _meta_language_code3 = (
            job_params.get("meta_language_code3") or channel.meta_language_code3
        )  # noqa: E272, E222
        _meta_video_source = (
            job_params.get("meta_video_source") or channel.meta_video_source
        )  # noqa: E272, E222
        ...
        subs = self._extract_subs_with_ccextractor(
            inp=filename_out, tp=_meta_teletext_page, date_from=date_from
        )
        ...
        with open(os.path.join(self.cwd, filename_txt), "w") as ftxt:
            ...
            _txt_header = (
                ...
                local_from_date="{} {}".format(
                    _LBT, _meta_timezone
                ),  # because ARROW can't handle ZZZ correctly
            )
...
def start(app_config):
    ...
    return {"kind": UploaderJobResultProcessor.KIND, "data": ret}
