	...
	* job_dir
    ...
    def create_command_from_config(self, command_config, job_params, timeout_sec=None):
        ...
        if isinstance(command_config, str):
            _cmd_params = {"cmd": command_config}
    ...
    def config(self):
        ...
        if main_cmd_config:
            command = self.create_command_from_config(
                main_cmd_config, self.job_params, self.duration_sec
            )
        ...
        if after_cmd_config_list:
            for after_cmd_config in after_cmd_config_list:
                command = self.create_command_from_config(
                    after_cmd_config, self.job_params
                )
                self.commands_list.append(command)

    def __str__(self):
        return "\n".join([str(cmd) for cmd in self.commands_list])
    ...
    def run(self):
        ...
        return {
            ...
            "data": {"job_ended": str(self.job_ID)},
        }
